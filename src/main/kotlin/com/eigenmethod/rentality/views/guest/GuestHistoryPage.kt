...
import com.eigenmethod.rentality.controllers.guest.getHistoryCarsForGuest
...
fun Container.guestHistoryPage(state: ConduitState) {
    setFoundHistoryCars()
                ...
                if (isHistoryCarsLoaded.value) {
                    if (historyCars.isEmpty()) {
                        ...
                        historyCars.forEach { historyCar ->
                            guestListingHistoryCar(historyCar)
                        }
                    }
                } else {
                    div("Loading ...")
...
private fun Container.guestListingHistoryCar(historyCar: BookedCar) {
    val car = historyCar.rentCar.car
    ...
    div {
        ...
        div(className = "relative overflow-hidden") {
            ...
            div("Pickup location: ${historyCar.rentCar.location}", className = "absolute top-[160px] left-6 bg-[$MAIN_COLOR_BG]/[$TRANSPARENCY_PERC] px-7 pt-1 pb-2 rounded-[30px] text-white text-base font-semibold")
            div("Total \$${historyCar.rentCar.getTotalPrice()}", className = "absolute top-[200px] left-6 bg-[$MAIN_COLOR_BG]/[$TRANSPARENCY_PERC] px-7 pt-1 pb-2 rounded-[30px] text-base font-semibold") {
            ...
            div(className = "cursor-pointer bg-[url('${car.imageURL.orEmpty()}')] bg-cover bg-no-repeat bg-center duration-500 hover:scale-[1.18] max-w-[570px] h-[280px] rounded-2xl") {
                onClick {
                    it.preventDefault()
                    routToHistoryDetailsPage(UserStatus.GUEST, historyCar)
                }
            }
        }
    ...
    coroutine.launch {
        withProgress {
            try {
                ...
                historyCars = getHistoryCarsForGuest()
